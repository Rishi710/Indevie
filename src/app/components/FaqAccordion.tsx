    ...
    <div className="w-full max-w-4xl mx-auto space-y-20 relative z-10">
      {categories.map((category, catIdx) => (
        <div key={catIdx} className="space-y-4">
          {/* <h2 className="text-2xl md:text-3xl font-inter text-gray-900 tracking-wide pb-2 uppercase">
          </h2> */}
            ...
            {category.items.map((item, itemIdx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className={`text-base md:text-xl font-medium pr-8 tracking-wide transition-colors duration-300 ${isOpen ? 'text-black' : 'text-black group-hover:text-bg-black'}`}>
                      {item.question}
                    </span>
                    <div className={`flex-shrink-0 relative w-4 h-4 flex items-center justify-center transition-colors duration-300 ${isOpen ? 'text-black' : 'text-gray-800 group-hover:text-black'}`}>
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="pb-8 pt-2 text-black text-sm font-inter font-light md:text-base leading-relaxed">
                          {item.answer}
                        </div>
                      </motion.div>
                    )}
