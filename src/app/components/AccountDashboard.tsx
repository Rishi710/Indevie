...
export default function AccountDashboard({ customer }: { customer: any }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-12 items-start">
      {/* Sidebar Navigation */}
      <aside className="w-full lg:flex-1 xl:max-w-[290px] lg:sticky lg:top-32">
            ...
            <nav className="space-y-2">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`w-full group relative flex items-center p-4 rounded-2xl transition-all duration-500 ${activeTab === tab.id
                    ? "bg-[#6c3518] text-white shadow-xl shadow-[#6c3518]/10"
                    : "text-[#6c3518]/60 hover:bg-[#f5f1e6]/50 hover:text-[#6c3518]"
                    }`}
                >
                  <div className={`p-2.5 rounded-xl transition-colors duration-500 ${activeTab === tab.id ? "bg-white/10" : "bg-gray-50 group-hover:bg-white"
                    }`}>
                    {tab.icon}
                  </div>
      ...
      {/* Main Content Area */}
      <main className="w-full lg:flex-[2.5] xl:flex-[3] min-h-[500px]">
