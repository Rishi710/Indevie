    ...
    <div className="space-y-6 lg:space-y-8">
      {state.success ? (
        <div className="space-y-6 text-center lg:text-left">
          <div className="bg-green-50 border border-green-100 text-[#4a7c59] px-6 py-5 rounded-2xl text-sm font-inter leading-relaxed">
          ...
          <Link
            href="/login"
            className="inline-block text-[11px] font-inter font-bold tracking-[0.2em] text-[#6c3518] uppercase border-b border-[#6c3518]/20 hover:border-[#6c3518] transition-all pb-1"
          >
            Return to Login
          </Link>
        </div>
        ...
        <form action={formAction} className="space-y-8">
          {state.error && (
            <div className="bg-red-50/80 backdrop-blur-sm border border-red-100 text-red-600 px-4 py-3 rounded-[8px] text-[13px] font-inter font-medium">
              {state.error}
            </div>
          )}

          <p className="text-gray-400 font-light font-inter text-xs lg:text-sm leading-relaxed">
            Enter your email address and we'll send you a link to reset your password and continue your skincare journey.
          </p>

          <div>
            <label htmlFor="email" className="block text-[10px] font-inter font-bold tracking-[0.2em] text-[#6c3518] uppercase mb-2">
              Email Address
            </label>
            <input
              type="email"
